...
// 仅替换 \n 为空格
static QString cleanLineBreak(const QString &src)
{
    QString res = src;
    res = res.replace("\\n", " "); // 界面显示的文字 \n
    res = res.replace("\n", " ");  // 真实换行符
...
void WordBookWidget::loadNextPage()
{
    ...
    for (int i = m_curLoadedCnt; i < end; ++i)
    {
        ...
        // 释义替换\n为空格
        QString meanText = cleanLineBreak(w["meaning"]);
        left->addWidget(new QLabel("释义：" + meanText));

        if (!w["example"].isEmpty())
        {
            QString exText = cleanLineBreak(w["example"]);
            left->addWidget(new QLabel("例句：" + exText));
        }
...
void WordBookWidget::showWordFullDetail(int wordId)
{
    ...
    QString meaning = cleanLineBreak(query->value(2).toString());
    QString example = cleanLineBreak(query->value(3).toString());
