...
void WordListWidget::loadNextPage()
{
    ...
    for (int i = m_curLoadedCnt; i < end; ++i) {
        ...
        // 将所有换行符替换为空格
        QString meanFlat = w["meaning"].replace("\n", " ").replace("\\n", " ");
        QLabel *meanLab = new QLabel("释义：" + meanFlat);
        meanLab->setWordWrap(true);
        ...
        btnWrapWidget->setFixedWidth(82);
...
void WordListWidget::onCalendarClick(const QDate &dt)
{
    ...
    while (query->next()) {
        ...
        QString meanFlat = meaning;
        // 循环替换，直到不再包含 "\n" 字符串
        while (meanFlat.contains("\\n"))
        {
            meanFlat = meanFlat.replace("\\n", " ");
        }
        // 顺带清除真实换行符
        meanFlat = meanFlat.replace("\n", " ");
        left->addWidget(new QLabel("释义：" + meanFlat));
        cl->addLayout(left);
        cl->addStretch();
...
void WordListWidget::showWordFullDetail(int wordId)
{
    ...
    // 删除词性行
    addLab(QString("释义：%1").arg(meanFlat));
